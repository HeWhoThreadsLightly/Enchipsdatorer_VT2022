...
char strDumpBuff[3000];
void dumpBuffer(char * dump, const uint32_t bytes, const uint32_t columnSets, const uint32_t * indicatorOffsets, const char * indicator, const int indicatorsOffsetsAreAdditive){
	char * str = strDumpBuff;
	uint32_t nextIndicatorOffset = 0;
	if(indicatorOffsets != NULL){
		nextIndicatorOffset = *indicatorOffsets;
	}
		...
		for(uint32_t columnSetI = 0; columnSetI < columnSets; columnSetI++){
			for(uint32_t j = i+16; i<j && i < bytes; i++){
				...
				if(indicatorOffsets == NULL){
					...
					if(nextIndicatorOffset == i){
						if(indicatorsOffsetsAreAdditive){
							nextIndicatorOffset += *indicatorOffsets;
						}else{
							nextIndicatorOffset = *indicatorOffsets;
						}
...
//#define vgaDebug // detailed debugging of main vga loop
//#define vgaLoop // use a loop instead of interrupts
void __attribute__((optimize("O3"))) vgaDriver(){
	...
	dumpBuffer(lineBuff, horiWhole*2, 2, {horiFront, horiSync, horiBack, horiRes, horiFront, horiSync, horiBack, horiRes}, "  []  ||  []  ||EE", 1);
